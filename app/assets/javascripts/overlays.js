	...
	for (var i = -blocks; i <= blocks; i++) {
		...
		for (var j = -blocks; j <= blocks; j++) {
			...
			for (var k = 0; k < listings.length; k++) {
				...
				pos = new google.maps.LatLng(listings[k]['lat'], listings[k]['lon']);
				price = listings[k]['price']

				if (squares[i+blocks][j+blocks].bounds.contains(pos)) {

					// ignore listings with null prices
					if (price == null) continue;

					// find min and max
					if (squares[i + blocks][j + blocks].min == null || price < squares[i + blocks][j + blocks].min) {
						squares[i + blocks][j + blocks].min = price;
					}
					if (squares[i + blocks][j + blocks].max == null || price > squares[i + blocks][j + blocks].max) {
